...
## 📣 Code of Conduct
This project and everyone participating are governed by this [Code of Conduct](CODE_OF_CONDUCT.md). Please familiarise yourself with what behaviour will not be tolerated, as we expect you to act accordingly to this code. You can report unacceptable behaviour to hello@khulnasoft.com. 

## 🐛 Reporting Bugs
Before creating a bug report, see if the problem [has already been reported](https://github.com/khulnasoft/<repository>/issues). If yes, and **the issue is still open**, add a comment to the existing issue instead of creating a new one.
...
Before suggesting a new feature, see if [has already been suggested](https://github.com/khulnasoft/<repository>/issues?q=is%3Aissue+is%3Aopen+Feature+request). If yes, and **the issue is still open**, add a comment to the existing issue instead of creating a new one.
